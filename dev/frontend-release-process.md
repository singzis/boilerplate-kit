...
- **主分支**：`main`/`master`（生产环境代码，保持稳定可部署状态）
- **开发分支**：`develop`（日常开发主分支，集成已完成的功能）
...
| 环节     | 推荐工具/服务                      | 作用                                |
| -------- | ---------------------------------- | ----------------------------------- |
| 分支管理 | GitLab/GitHub                      | 分支权限控制、PR 评审、冲突解决     |
| 版本管理 | standard-version、npm version      | 自动生成 CHANGELOG、管理版本号      |
| CI/CD    | GitHub Actions、GitLab CI、Jenkins | 自动化构建、测试、部署              |
| 构建优化 | webpack-bundle-analyzer、esbuild   | 分析包体积、提升构建速度            |
| 灰度发布 | Nginx、阿里云 MSE、云效流量管理    | 控制流量比例、实现平滑发布          |
| 监控告警 | Sentry（错误）、New Relic（性能）  | 实时捕获异常、触发告警（邮件/钉钉） |
| 日志分析 | ELK Stack、阿里云 SLS              | 收集前端日志、排查线上问题          |

## 五、注意事项

1. **发版时间选择**：避开业务高峰期（如电商平台的晚间、促销活动期间），预留回滚时间（如工作日上午）。
2. **自动化优先**：核心步骤（构建、部署、测试）尽量自动化，减少人工操作失误。
3. **回滚预案**：提前准备回滚方案（如通过 Tag 部署上一版本、CDN 回源旧资源），确保 10 分钟内可执行回滚。
4. **权限控制**：主分支（`main`）禁止直接提交，需通过 PR 合并并经审核；部署生产环境需特定权限（如运维或负责人）。
5. **沟通同步**：发版前 1 小时在团队群通知（包含版本号、变更内容、预计时间），发布完成后同步结果。
